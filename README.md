...
From inside the `gc-content-analysis/` directory, run:

```
pip install -r requirements.txt
```

### Step 2 — Run the pipeline on the first dataset

```
kedro run
```

Open `data/07_model_output/results.csv` and note the `gc_content` values for each sequence.

### Step 3 — Swap the data (the key step)

Open `conf/base/catalog.yml` and change **one word**:
